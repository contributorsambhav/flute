...
const NFTMarketplace: React.FC<NFTMarketplaceProps> = ({
  ...
  const buyNFT = async (nft: NFT): Promise<void> => {
    ...
    try {
      ...
      alert(
        `NFT purchased successfully! Transaction hash: ${data.transactionHash}`,
      );
      ...
      alert(
        `Error purchasing NFT: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredNFTs.map((nft: NFT) => (
          <div
            key={nft.id}
            className="bg-white/5 rounded-lg p-2 border-white/20 hover:bg-white/10 transition-all duration-300 group"
          >
            <div className="p-0">
              <div className="relative overflow-hidden rounded-xl">
                <img
                  src={nft.image}
                  alt={nft.title}
                  className="w-full h-64 object-cover group-hover:scale-101 transition-all duration-300 ease-out"
                />
            ...
            <div className="p-4">
              <div className="text-white text-lg mb-2">{nft.title}</div>
            ...
            <div className="p-4 pt-0">
              <Button
                className="w-full bg-neutral-700 hover:bg-neutral-700"
                onClick={() => buyNFT(nft)}
