...
const CreateNFT: React.FC<CreateNFTProps> = ({ isConnected, account }) => {
  ...
  const mintNFT = async (): Promise<void> => {
    ...
    try {
      const category =
        creationMode === "generate" ? "ai-generated" : "uploaded";
      ...
      alert(
        `Error minting NFT: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setIsMinting(false);
    }
    ...
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center mb-2 sm:mb-4">
          <Palette className="mr-2 sm:mr-3 h-5 w-5 sm:h-6 sm:w-6 text-purple-400" />
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white">
            Create NFT
          </h1>
        </div>
        <p className="text-sm sm:text-base text-white/70 max-w-2xl">
          Upload your own artwork or generate unique art using AI to create
          NFTs.
        </p>
      </div>
